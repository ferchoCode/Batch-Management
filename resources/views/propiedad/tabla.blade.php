            ...
            <tbody class="border border-1">
                @foreach ($propiedad as $propiedades)
                    <tr >
                        ...
                        <td class="border border-1">{{ $propiedades->cod }}</td>
                        <td class="border border-1">
                            <img src="{{ asset($propiedades->fotografia) }}" alt=""
                                class="img-fluid img-thumbnail" width="200px">
                        </td>
                        <td class="border border-1">{{ $propiedades->nombre }}</td>
                        <td class="border border-1">{{ $propiedades->direccion }}</td>
                        <td class="border border-1">{{ $propiedades->precio }}</td>
                        <td class="border border-1"><span class="badge bg-success text-white"> {{ $propiedades->estado }}</span></td>
                        <td class="border border-1">{{ $propiedades->propietarioN }}  {{ $propiedades->propietarioA }}</td>
                        <td class="border border-1" >{{ $propiedades->vendedorN }}  {{ $propiedades->vendedorA }}</td>

                        <td>
                            <form id="eliminar{{ $propiedades->id }}"
                                action="{{ url('propiedad/delete/' . $propiedades->id) }}" method="POST">
                                @csrf
                                {{--                
                        <a  href="{{ url('cuota/'.$propiedades->id) }}"  class="btn  btn-icon btn-warning btn-sm mr-2">
                             Cuotas
                                ...
                                <button type="button" onclick="confirmEliminar({{ $propiedades->id }})"
