            ...
            <tbody class="border border-1">
                @foreach ($reservas as $reserva)
                    <tr>
                        ...
                        <td class="border border-1">{{ $reserva->nombre_completo }}</td>
                        <td class="border border-1">{{ $reserva->lote }}</td>
                        <td class="border border-1">{{ $reserva->monto }}</td>
                        <td class="">{{ $reserva->fecha_reserva }}</td>
                        ...
                        @if ($reserva->estado == 'Activa')
                            ...
                            <td>
                            @else
                            <td class=""><span class="badge bg-danger text-white">
                                    {{ $reserva->estado }}</span></td>
                            <td>
                        @endif


                        <form id="eliminar{{ $reserva->id }}" action="{{ url('reserva/delete/' . $reserva->id) }}"
                            method="POST">
                            @csrf
                            @if ($reserva->estado == 'Activa')
                                <a href="{{ url('reserva/' . $reserva->id . '/reservaventa') }}"
                                    class="btn  btn-icon btn-success btn-sm mr-2">
                                    <iclass="fa-solid fa-list"></i>Realizar venta
                                </a>

                                <button type="button" onclick="confirmEliminar({{ $reserva->id }})"
