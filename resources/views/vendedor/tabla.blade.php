        ...
        <table>
            ...
            <tbody class="border border-1">
                @foreach ($vendedor as $vendedores)
                    <tr >
                        <td class="border border-1">{{ $vendedores->ci }}</td>
                        <td class="border border-1">{{ $vendedores->nombre }}</td>
                        <td class="border border-1">{{ $vendedores->apellido }}</td>
                        <td class="border border-1">{{ $vendedores->telefono }}</td>
                        <td class="border border-1">{{ $vendedores->direccion }}</td>

                        <td>
                            <form id="eliminar{{ $vendedores->id }}"
                                action="{{ url('vendedor/delete/' . $vendedores->id) }}" method="POST">
                                @csrf
                                {{--                
                        <a  href="{{ url('cuota/'.$propiedades->id) }}"  class="btn  btn-icon btn-warning btn-sm mr-2">
                             Cuotas
                                ...
                                <button type="button" onclick="confirmEliminar({{ $vendedores->id }})"
