            ...
            <tbody class="border border-1">
                @foreach ($paquete as $paquetes)
                    <tr>
                        ...
                        <td class="border border-1">{{ $paquetes->nombre }}</td>
                        <td class="border border-1">{{ $paquetes->descripcion }}</td>
                        <td class="border border-1">{{ $paquetes->precio }}</td>
                        <td class=""><span class="badge bg-success text-white">
                                {{ $paquetes->estado }}</span></td>
                        <td class="">{{ $paquetes->fecha_inicio }}</td>
                        <td>{{ $paquetes->fecha_fin }}</td>



                        <td>
                            <form id="eliminar{{ $paquetes->id }}"
                                action="{{ url('paquete/delete/' . $paquetes->id) }}" method="POST">
                                @csrf
                                {{--                
                        <a  href="{{ url('cuota/'.$propiedades->id) }}"  class="btn  btn-icon btn-warning btn-sm mr-2">
                             Cuotas
                        </a> --}}
                                <a href="{{url('paquete/'.$paquetes->id.'/paqueteservicio')}}" class="btn  btn-icon btn-success btn-sm mr-2"><i
                                        class="fa-solid fa-list"></i></a>

                                <button type="button" onclick="confirmEliminar({{ $paquetes->id }})"
